...
- Blender 5.1.2 is now installed locally and the fork has a first Blender/GLB authoring pipeline. The initial movement gym source scene is `experiments/neomud-three/assets/source/levels/movement_gym.blend`; its exported runtime GLB is `experiments/neomud-three/assets/build/levels/movement_gym.glb`. The first main-runtime room package is `town:temple`, with source image/brief under `experiments/neomud-three/assets/source/scenes/town_temple/`, Blender source `town_temple.blend`, and exported runtime GLB `experiments/neomud-three/assets/build/levels/town_temple.glb`.
- `scripts/validate-neomud-three-gltf.mjs` now has profile-aware validation for movement gyms, rooms, and landmarks. Room packages must export `VIS_`, `COL_`, `SPAWN_`, `TRG_`, `CAMERA_`, and `LIGHTS_` metadata with usable spawn/collision/trigger extras.
...
- `experiments/neomud-three/glb-room-runtime.js` now turns a parsed GLB room package into the standard playable room runtime contract: spawn, bounds clamp, GLB-derived collider pushout, physical exit triggers, debug colliders, debug triggers, and package landmark metadata. Temple uses this adapter instead of local room-specific GLB parsing glue.
...
- The GLB package carries visible cathedral geometry, collision boxes, player spawn, north exit trigger to `town:square`, camera hint, and light marker through `VIS_`, `COL_`, `SPAWN_`, `TRG_`, `CAMERA_`, and `LIGHTS_` prefixes.
- The playable cathedral still has a long nave, pew rows, side columns, stained glass bands, a south altar/retable, incense braziers, and a north doorway back to Town Square.
...
- The current GLB art is intentionally first-pass and low-poly. It proves the production workflow, not final cathedral art. The next Temple pass should refine mesh proportions, material IDs, and authored lighting inside Blender rather than returning to runtime mesh construction.
...
- The latest Barkeep staging pass adds a low-cost service spot, Barkeep board, tap handles, tray/ledger details, and a warmer service light so the bar reads as the main interaction affordance in the Tavern bar screenshot.
- The latest batching pass converts repeated Tavern mugs and table/bench parts into instanced meshes, reclaiming draw-call headroom while preserving the same layout, collision, screenshots, and server-backed traversal behavior.
- The latest tabletop warmth pass adds instanced table runners, plates, mugs, and candle details across the existing tables. It makes the room feel less empty while preserving table collision and Tavern traversal.
...
- Player character art is still unresolved for final quality. The v6 compact proxy is more coherent with the low-poly diorama and dramatically cheaper than visible Xbot, but it is still placeholder art rather than a proper authored NeoMud character.
...
- `scripts/test-neomud-three.cjs` runs a local browser smoke test against the offline Three renderer at `?offline=1`. It now asserts Temple's Blender GLB landmark metadata, north exit trigger, and altar collider before toggling the playable-room debug overlay in Town Square, capturing `offline-town-square-debug.png`, asserting collider/trigger/entity debug coverage, and turning the overlay off before normal budget assertions.
- `scripts/test-neomud-three-labs.cjs` runs Material Lab, Prop Zoo, and Movement Gym browser QA, screenshots all three lab levels, checks render budgets, verifies approved material/prop metadata, and asserts the Blender GLB parser summary plus Movement Gym debug-layer coverage.
...
- Latest offline smoke after the Blender Temple migration reports Temple 220 calls / 4,656 triangles / 1 texture / 171 geometries; Town Square 211 calls / 21,280 triangles / 15 textures / 161 geometries; Market Street 133 calls / 6,738 triangles / 21 textures / 73 geometries; Magic Shop 58 calls / 6,704 triangles / 25 textures / 47 geometries; Forge 58 calls / 4,364 triangles / 28 textures / 45 geometries; North Gate 37 calls / 3,758 triangles / 31 textures / 29 geometries; Forest Edge 43 calls / 4,810 triangles / 34 textures / 36 geometries; Forest Path 45 calls / 5,236 triangles / 38 textures / 34 geometries; Deep Forest 46 calls / 3,914 triangles / 42 textures / 34 geometries; Hidden Cave 54 calls / 3,321 triangles / 45 textures / 44 geometries; Sunlit Clearing 45 calls / 4,681 triangles / 47 textures / 34 geometries; Tavern 90 calls / 9,302 triangles / 50 textures / 66 geometries.
- Latest server-backed QA reports Temple 220 calls / 4,656 triangles / 1 texture / 171 geometries; Town Square 211 calls / 21,280 triangles / 15 textures / 161 geometries; Market Street 133 calls / 6,738 triangles / 19 textures / 73 geometries; Magic Shop 58 calls / 6,704 triangles / 23 textures / 47 geometries; Forge 58 calls / 4,364 triangles / 26 textures / 45 geometries; North Gate 37 calls / 3,758 triangles / 31 textures / 29 geometries; Forest Edge 43 calls / 4,810 triangles / 33 textures / 36 geometries; Forest Path 42 calls / 4,322 triangles / 36 textures / 32 geometries; Deep Forest 41 calls / 2,996 triangles / 39 textures / 32 geometries; Hidden Cave 54 calls / 3,321 triangles / 42 textures / 44 geometries; Sunlit Clearing 48 calls / 5,461 triangles / 44 textures / 37 geometries; Tavern 89 calls / 9,290 triangles / 48 textures / 66 geometries.
- Latest Town Square screenshot-anchor QA reports 114 calls / 15,418 triangles / 20 textures / 181 geometries, no console errors, no failed requests, and a passing budget report.
- Latest authored-room screenshot QA after the Blender Temple migration reports Temple 212 calls / 4,512 triangles / 1 texture / 164 geometries; Town Square 208 calls / 21,264 triangles / 15 textures / 161 geometries; Market Street 133 calls / 6,738 triangles / 19 textures / 73 geometries; Magic Shop 58 calls / 6,704 triangles / 23 textures / 47 geometries; Forge 58 calls / 4,364 triangles / 26 textures / 45 geometries; North Gate 37 calls / 3,758 triangles / 29 textures / 29 geometries; Forest Edge 43 calls / 4,810 triangles / 31 textures / 36 geometries; Forest Path 45 calls / 5,236 triangles / 35 textures / 34 geometries; Deep Forest 46 calls / 3,914 triangles / 39 textures / 34 geometries; Hidden Cave 54 calls / 3,321 triangles / 42 textures / 44 geometries; Sunlit Clearing 45 calls / 4,681 triangles / 44 textures / 34 geometries; Tavern 90 calls / 9,302 triangles / 47 textures / 66 geometries, with no console errors or failed requests.
...
- Latest Prop Zoo QA reports 286 draw calls, 6,529 triangles, 47 textures, 184 geometries, no console errors, no failed requests, and a passing budget report. The Prop Zoo review-scene budget was explicitly raised to 340 calls / 120k triangles / 56 textures / 240 geometries after adding reusable civic props.
