...
- `memory-bank/asset-level-qa-workflow.md` now defines the corrective workflow for cathedral fixtures: isolated Blender source/export, browser asset-lab QA, then room integration only after the fixture proves itself. The first target is `cathedral.pew`.
...
- The latest fixture pass removes the obstructing freestanding side-pier/lintel geometry that sat visually in front of stained-glass windows and replaces it with shallow wall ribs. The pews now reuse an isolated `cathedral.pew` asset candidate with separate seat, sloped back, rails, shaped end panels, trim, an asset manifest, and a browser asset-lab QA page.
...
- The current GLB art is still low-poly and not final cathedral art. The next Temple pass should continue fixture-level QA, especially the wall-window bay, altar/retable, and incense brazier, rather than returning to broad room-wide mesh construction.
...
- `scripts/test-neomud-three-labs.cjs` also screenshots `cathedral-asset-lab.html` and asserts the isolated `cathedral.pew` candidate has separate backrest/end-panel render nodes plus a collider footprint.
...
- Latest Cathedral Asset Lab QA reports 112 draw calls, 1,726 triangles, 6 textures, 108 geometries, no console errors, no failed requests, and a passing budget report for the first `cathedral.pew` candidate.
