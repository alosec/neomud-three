...
- The Blender movement gym is validated but not yet loaded by the Three runtime. The next architecture pass should add a GLB loader/parser that turns `VIS_`, `COL_`, `SPAWN_`, `TRG_`, `PICKUP_`, `ENEMY_`, `PATH_`, `CAMERA_`, and `LIGHTS_` nodes into render objects and typed gameplay metadata.
