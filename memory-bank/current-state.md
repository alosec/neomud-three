...
- The Blender movement gym is loaded by a lab page, but not yet integrated into the main room runtime, movement controller, collision system, or server-backed room graph. The next architecture pass should make the parser output usable by the main renderer and add debug draw toggles for parsed colliders/triggers/spawns.
...
- `scripts/test-neomud-three-labs.cjs` runs Material Lab, Prop Zoo, and Movement Gym browser QA, screenshots all three lab levels, checks render budgets, verifies approved material/prop metadata, and asserts the Blender GLB parser summary.
