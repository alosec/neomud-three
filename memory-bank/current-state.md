...
- The player avatar now renders as `procedural-adventurer-proxy-v8`: a smaller low-poly fantasy adventurer proxy with narrower/shorter cloak panels, separated hood/face, visible hands, darker cloak/tunic palette, shoulder pieces, trim, staff/orb, satchel, boots, chest strap, back gear, hair/skin detail, and small steel equipment accents. The vendored Three.js/Xbot GLTF still loads as a hidden technical/reference animation asset, but it is no longer rendered by default because the visible example mesh dominated room triangle budgets. This is a better scale/silhouette bridge, not final accepted player art.
...
- Player character art is still unresolved for final quality. The active v8 articulated procedural proxy is smaller and less cloak-dominant than v7, and it is reviewed in Avatar Lab, but it is still placeholder art rather than a proper authored NeoMud character.
