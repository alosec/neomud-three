...
- The main client intentionally supports two visual modes, not one discarded
  ...
  pointer on open ground continuously updates the movement destination until
  ...
  Iso click-to-move keeps destination placement compatible with existing room
  collision pushout: clicks that land inside blocked civic props such as the
  ...
  Room debug entity metadata is now serialized through one shared helper so
  action type, item id, coin type, quantity, prompt, and position stay available
  consistently across authored rooms and QA scripts.
...
- The latest outdoor horizon pass batches those four painted horizons into one shared horizon system. It improves the edge-of-map read with layered moss terraces, tree bands, quiet background buildings, and roof hints while lowering full offline Town Square draw calls from 238 to 224. Town Square remains triangle-hot at roughly 78.9k/80k, so future visual work should reclaim triangles before adding more visible objects.
...
- The current runtime is still primarily tuned around third-person/chase camera
  ...
  this is still a transitional state until pathing, combat targeting, broader
  selected-target behavior across rooms, and richer ARPG affordance language
  are implemented.
...
- Player character art is still unresolved for final quality. The active Xbot model is an animated placeholder with a cleaner teal material treatment, and it is reviewed in Avatar Lab. A rigid costume overlay attempt was rejected because it produced floating/bulky artifacts in side and jump poses. The next real character step should be a proper sourced/authored skinned fantasy adventurer GLB.
...
- Latest Town Square staging pass raises/narrows context-tree canopies and tightens benches, lamps, and string lights into a clearer civic ring while keeping the cardinal exit lanes open. Town Square screenshot-anchor QA reports 135 calls / 25,976 triangles / 21 textures / 209 geometries, no console errors, no failed requests, and a passing budget report.
- Forest Edge, Forest Path, Sunlit Clearing, and Deep Forest now render simple physical exit gateways from existing trigger metadata so exits read as framed openings instead of only floating labels. Hidden Cave intentionally keeps its smaller cave-specific label for now because the generic gateway frame occluded the close entry camera and needs a dedicated tunnel-mouth pass.
- Town Square now applies a 1.18x horizontal runtime spec scale so landmarks, paths, colliders, triggers, entity placements, and spawn points spread outward together without adding more detail density. Latest Town Square screenshot-anchor QA reports 135 calls / 25,936 triangles / 21 textures / 209 geometries, no console errors, no failed requests, and a passing budget report.
- Tavern now has a source-level Blender scale pass: the room footprint is 28x22, tables and stools are spread outward, the bar/door opening are widened, and the camera is pulled back. The GLB package and manifest were regenerated from `scripts/create-neomud-three-town-tavern.py`; room-shot QA, package validation, and offline smoke pass.
- Temple now has a source-level aisle breathing-room pass: pews are narrower, pew rows sit farther from the center line, and the runner is narrower. The GLB package and manifest were regenerated from `scripts/create-neomud-three-town-temple.py`; room-shot QA, scenic review, package validation, and offline smoke pass.
