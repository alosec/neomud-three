...
- `scripts/test-neomud-three.cjs` runs a local browser smoke test against the offline Three renderer at `?offline=1`. It now toggles the playable-room debug overlay in Town Square, captures `offline-town-square-debug.png`, asserts collider/trigger/entity debug coverage, and turns the overlay off before normal budget assertions.
