...
2. Refine the Blender-authored Temple of the Dawn package.
   ...
   - Do not accept the current altar and incense as final art. Smoke has moved from hard cones to subtler low-poly puff columns, pews now have a stronger second candidate, and Temple has a small runtime light rig. The next Temple fixture/material pass should improve altar material hierarchy, reconcile authored `LIGHTS_` markers with runtime lighting, add shared wood/glass material variants, or optimize texture delivery with screenshot evidence.
