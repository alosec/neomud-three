...
0b. Shift the play model toward Diablo 3-inspired controls and flow.
   ...
   - First obstacle-routing slice exists: if a direct Iso click path crosses a
     ...
     advances early to the farthest visible remaining waypoint near corners,
     reducing hard stop-and-turn motion without cutting through colliders. Click
     routes now auto-run in Iso mode and include a small stuck/repath guard that
     recomputes from the current avatar position if collider clamping prevents progress.
   ...
   - Direct click routing now covers existing NPC/item interaction panels and
     ...
     avatar toward the object, and open the interaction panel only once the
     avatar reaches interaction range. Exit clicks now use the same game-like
     approach flow: the client stores `pendingExit`, shows the normal
     movement/path preview, walks to the trigger, and only then dispatches the
     room move. Town Square physical exit boards now carry exit target metadata
     and are raycast-clickable surfaces, with QA covering a direct Gate board
     click before trigger-volume fallback. Next implementation should extend
     this object-level clickability to other room gateways and authored
   ...
   - Selection feedback now has first-pass object response through target-group
     ...
     `Smite -n` instead of generic damage-only text. Spell effects now resolve
     catalog school/id metadata and render a school-colored impact ring; skill
     effects render a distinct physical burst, with latest-effect metadata
     exposed for QA. Action details now include catalog cooldown values such as
